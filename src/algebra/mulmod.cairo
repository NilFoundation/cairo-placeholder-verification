...
func math_mulmod{range_check_ptr}(x: Uint256, y: Uint256, k: Uint256) -> (res: Uint256) {
    alloc_locals;
    if (k.high + k.low == 0) {
    ...
    let (xy_high_exp_mod_k) = math_mulmod(exp_mod_k, xy_high_mod_k, k);
    let (res) = math_addmod(xy_high_exp_mod_k, xy_low_mod_k, k);
