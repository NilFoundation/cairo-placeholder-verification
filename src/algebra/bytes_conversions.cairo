...
from algebra.pow2 import pow2, u256_pow2
...
func math_bytes_widen(x: felt, widthDiff: felt) -> (res: felt) {
    let (multiplier) = pow2(widthDiff);
    return (x * multiplier,);
}

func math_bytes_widen_256{range_check_ptr}(x: felt, widthDiff: felt) -> (res: Uint256) {
...
func math_bytes_narrow{bitwise_ptr: BitwiseBuiltin*}(x: felt, widthDiff: felt) -> (res: felt) {
...
func math_bytes_narrow_256{range_check_ptr: felt}(x: Uint256, widthDiff: felt) -> (res: felt) {
